...
% Inclusions created in epi{1} (first region deonted by {1})
epi{1} = ones(nx{1},ny{1})*c_eps_0;
epi{1}(125:2:150,15:135) = c_eps_0 * 11.3;
epi{1}(1:2:26, 15:135) = c_eps_0 * 11.3;
%epi{1}(80:100, 1:40) = c_eps_0 * 11.3;
%epi{1}(80:100, 110:150) = c_eps_0 * 11.3;
...
bc{1}.NumS = 2;
bc{1}.s(1).xpos = nx{1}/(4) + 1; % Starting x position for the first pulse
bc{1}.s(1).type = 'ss';
bc{1}.s(1).fct = @PlaneWaveBC;
bc{1}.s(2).xpos = nx{1}/(4) + 1; % Starting x position for the second pulse
bc{1}.s(2).type = 'ss';
...
st = 15e-15;
s = 0;
y0_1 = yMax/4; % Starting y position for the first pulse
y0_2 = 3 * yMax / 4; % Starting y position for the second pulse
sty = 1.5*lambda;
